...
public interface ICarbonService
{
    ...
    /// <summary>Calculates how much carbon was lost due to bales being exported off field.</summary>
    void CalculateCarbonLostFromHayExports(Farm farm, CropViewItem cropViewItem);

    /// <summary>Equation 11.3.2-4</summary>
    ...
    /// <summary>Totals the aboveground carbon input. (kg C ha^-1)</summary>
    double SumTotalAbovegroundCarbonInput(List<CropViewItem> viewItems);

    /// <summary>Totals the belowground carbon input. (kg C ha^-1)</summary>
    double SumTotalBelowgroundCarbonInput(List<CropViewItem> viewItems);

    /// <summary>Totals the manure carbon input. (kg C ha^-1)</summary>
    double SumTotalManureCarbonInput(List<CropViewItem> viewItems);

    /// <summary>Totals the digestate carbon input. (kg C ha^-1)</summary>
    double SumTotalDigestateCarbonInput(List<CropViewItem> viewItems);

    /// <summary>
    /// Combines C inputs for all items by year. This combines the inputs from the main crop grown that year plus the cover crop (if specified by user).
    /// Must be called after all inputs from crops, manure, etc. have been calculated for each detail view item since we simply add up the total above and
    /// below ground C inputs for each year. Inputs from the secondary crop are added to the main crop since the main crop view item will be used in the
    /// final ICBM/IPCC Tier 2 calculations.
    /// </summary>
    void CombineCarbonInputs(Farm farm,
        List<CropViewItem> viewItems);

    // Note: command-line residue-input recalculation lives on FieldResultsService
    // (ProcessCommandLineItems), which reuses that service's existing Assign* initialization
    // methods, rather than here on CarbonService.
